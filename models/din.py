...
class _Dice(nn.Module):
    r"""
    math: $ p(x) = \frac{1}{1 + e^{-\frac{x-\mathbb{E}[x]}{\sart{Var(x) + \epsilon}}}}$
          $ f(x) = \alpha (1 - p(x)) \cdot x + p(x) \cdot x$
    """

    def __init__(self, input_dims, dim=-1, epsilon=1e-9):
        # dim : reduction on
        super(_Dice, self).__init__()
        self.alphas = nn.Parameter(torch.zeros(input_dims[-1], dtype=torch.float32))
        self.beta = nn.Parameter(torch.zeros(input_dims[-1], dtype=torch.float32))
        self.epsilon = epsilon

    def forward(self, x):
        mean = torch.mean(x, dim=[])  # TODO dim
        std = torch.std(x, dim=[])
        norm_x = (x - mean) / (std + self.epsilon)
        p_x = torch.sigmoid(norm_x)
        return self.alphas * (torch.tensor(1.) - p_x) * x + p_x * x
...
class MLP(nn.Module):
    def __init__(self, input_size, hidden_size, bias,
                 batch_norm=True, dropout_rate=0.5, activation='relu',
        ...
        layers = [nn.Linear(input_size, hidden_size[0], bias=bias[0])]
        ...
        self.fc = nn.Sequential(*layers)  # DNN
        if self.sigmoid:
...
class LocalActivationUnit(nn.Module):
    def __init__(self, hidden_size=(80, 40), bias=(True, True), embedding_dim=4, batch_norm=False):
        ...
        self.fc1 = MLP(input_size=4 * embedding_dim,
        ...
        self.fc2 = MLP(input_size=hidden_size[-1],
                       hidden_size=[1],
                       bias=[True],
                       batch_norm=batch_norm,
                       activation='dice',
...
class DeepInterestNetwork(nn.Module):
    def __init__(self, config):
        ...
        self.query_image_fc = MLP(input_size=2048,
        ...
        for feature in his_embed_features:
            self.history_feature_embedding_dict[feature] = EmbeddingLayer(feature_dim=dim_config[feature],
                                                                          embedding_dim=embedding_size).to(device)
        self.history_image_fc = MLP(input_size=2048,
                                    hidden_size=[image_hidden_dim],
                                    bias=[True],
                                    activation='relu').to(device)

        self.attention = AttentionSequencePoolingLayer(
            embedding_dim=image_hidden_dim + embedding_size + category_dim).to(device)
        self.fc_layer = MLP(input_size=2 * (image_hidden_dim + embedding_size + category_dim) + sum(
    ...
    def forward(self, user_features):
        ...
        # print('Query feature_embed size', query_feature_embedded.size())
        ...
        # batch_size * T * (feature_size * embedding_size)
        # print('History feature_embed size', history_feature_embedded.size())
        # print('History feature done')

        # print(user_features['history_len'])
        # print(user_features['history_len'].size())

        history = self.attention(query_feature_embedded.unsqueeze(1),
                                 history_feature_embedded,
...
if __name__ == "__main__":
    din = DeepInterestNetwork(config={'embedding_size': 32})

    user_feature = {
        'user_exposed_time': torch.zeros(size=(2, 24), dtype=torch.long),
        'user_gender': torch.zeros(size=(2, 2), dtype=torch.long),
        'user_age': torch.zeros(size=(2, 9), dtype=torch.long),
        'query_article_id': torch.zeros(size=(2, 9), dtype=torch.long)
    }
    din(user_feature)
